        ...
        sniffer.sniff_loop([=](PDU& pdu) {
            ...
            switch (pdu.rfind_pdu<IP>().inner_pdu()->pdu_type())
            {
                case PDU::PDUType::TCP : {
                    myTcp = pdu.rfind_pdu<TCP>();
                    cout << "TCP" << endl;                    
                    EthernetII new_eth = EthernetII(toHw,info.hw_addr)/IP(to,myIp)/ myTcp/pdu.rfind_pdu<RawPDU>();
                    break;
                }

                case PDU::PDUType::UDP :{
                    cout << "UDP" << endl;
                    newEth = EthernetII(toHw,info.hw_addr)/IP(to,myIp)/pdu.rfind_pdu<UDP>();
                    break;
                }
            ...
            this->arpSender.send(newEth,this->iface);
            return true;
        });
}
